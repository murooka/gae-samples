...
```bash
mvn package
java -jar target/helidon-graal.jar
```
...
```
docker build -t helidon-graal .
```

## Start the application with Docker

```
docker run --rm -p 8080:8080 helidon-graal:latest
```
...
kubectl get service helidon-graal  # Get service info
```
...
```
docker build -t helidon-graal-native -f Dockerfile.native .
```

Start the application:

```
docker run --rm -p 8080:8080 helidon-graal-native:latest
```